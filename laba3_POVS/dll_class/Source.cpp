...
void AsciiDocEditor::PrintHeadings() {
	...
	if (headersBegin == headersEnd) {
		throw "Заголовков не найдено.";
		return;
	}
	std::cout << "Заголовки:" << std::endl;
	for (std::sregex_iterator i = headersBegin; i != headersEnd; ++i) { // ++i vs i++
...
void AsciiDocEditor::PrintParagraphs()
{
	...
	if (paragraphsBegin == paragraphsEnd) {
		throw "Абзацев не найдено.";
		return;
	}
	std::cout << "Абзацы:" << std::endl;
...
void AsciiDocEditor::PrintBoldText()
{
	...
	if (boldsBegin == boldsEnd) {
		throw "Жирного текста не найдено.";
		return;
	}
	std::cout << "Жирный текст:" << std::endl;
